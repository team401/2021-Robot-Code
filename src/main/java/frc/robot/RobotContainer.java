...
public class RobotContainer {
    ...
    public RobotContainer() {
        ...
        drive.setDefaultCommand(
            new OperatorControl(
                drive,
                () -> leftJoystick.getY(GenericHID.Hand.kLeft) / 2,
                () -> leftJoystick.getX(GenericHID.Hand.kLeft) / 2,
                () -> rightJoystick.getX(GenericHID.Hand.kRight) / 2,
                true
            )
        );
        ...
        shooter.setDefaultCommand(
            new InstantCommand(
                () -> shooter.runShooterPercent(gamepad.getRawAxis(3) / 5), 
                shooter
            )
        );
